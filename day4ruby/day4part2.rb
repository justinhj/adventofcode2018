...
guards_sleep_minutes.each do |g, mins|
  mins = mins.sort_by { | k, _| k }
  fave_minute, times = mins.max_by { |_, v| v }

  # print "guard #{g} fave min #{fave_minute} times #{times}\n"
  guards_to_minute_times[g] = [fave_minute, times]
end

guard, minute_times = guards_to_minute_times.max_by { |_, m| m[1] }

fave_minute, times = minute_times
answer = guard.to_i * fave_minute

print "Answer is Guard ##{guard} at minute #{fave_minute} (#{times} times) : #{answer}\n"
